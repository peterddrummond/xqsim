...
%   g.limit{n}=[tl,tu;xl,xu;,..] is the n-th plot limit matrix
...
[sz1,sz2] = size(limit);                %%Get size of plot limits
lx = length(ax);                        %%Get length of axis vector
axis = gca;                             %%Get current axis handle

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   Test limit array size is OK

if sz2 ~= 2
error('Plot %d has 2nd limit dimension of %d, should be 2\n',n,sz2);
end 
 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   FIRST COORDINATE PLOT LIMITS       

if sz1 >= ax(1) && ax(1) > 0            %%If limit exists   
...
if sz1 >= ax(2) && ax(1) > 0            %%If limit exists   
...
if lx > 2 && sz1 >= ax(3) && ax(3) > 0  %%If axis and limit exists   
