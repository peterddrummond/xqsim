function   xchisq(n,d,g) 
%   XCHISQ(n,d,g) plots chi-squared data  
...
%   Can make one-sided or two-sided chi-square comparisons
...
if g.chisqplot{n} == 2                           %%First plot type?
    chisqu = ((chisqu./valid).^(1/3))-(1-2./(9*valid));
    chisqu = chisqu.*sqrt(valid.*4.5);           %%subtract count data
...
if g.chisqplot{n} == 1                           %%First plot type?
    ...
    g.olabels{n}='z_{\chi^2}';                   %%make label
end                                              %%End if plot type
