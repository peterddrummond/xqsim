function C = Nnslc(p)
% C = NNSLC(p); generates comparison count probabilities for an exact lossy
% pure squeezed state n-fold partition. Only valid for pure squeezed states
% with uniform input squeezing + Identity transmission matrix
if p.thermal ~= 0
    error('Nnlsc uses uniform pure squeezed state inputs');
elseif p.tr(1) == 1
    error('Nnlsc is for lossy distributions only');
end
partition = p.part{p.noutput};                   %current graph partition
obs = p.noutput;
xkcell = p.xk{obs};                              %count numbers for graph
if isempty(xkcell)                               %if no count numbers input
    xkcell{1} = 0:p.modes;                       %set counts to modes
end
maxm = 1;                                        %extract relevant counts
nd = length(partition);                          %partition dimension
for i =1:nd
    maxm = max(maxm,1+max(xkcell{i}));
end
sz = ones(1,nd)*maxm;                            %size of probability data                                     
C = zeros([sz,1]);                               %initialize total probability
p1 = zeros([1,nd]);                              %initialize numbers
M  = zeros([1,nd]);                              %initialize sizes
epC = zeros(maxm,1);                             %initialize even count product
opC = zeros(maxm,1);                             %initialize odd count product

if iscell(partition)                             %check if cell partition
    for j =1:nd                                  %loop on partitions
        i = partition{j}(1);                     %first partition index
        p1(j)  = 1/(1+(sinh(p.sqz(i))).^2);      %thermal numbers
        M(j)  = length(partition{j});            %partition length
    end                                          %end loop on partitions
else                                             %if partition vector
    i = 1;                                       %first detector index
    M = partition;                               %partition lengths
    for j=1:nd                                   %loop on vector index         
        p1(j) = 1/(1+(sinh(p.sqz(i))).^2);       %thermal numbers
        i=i+M(j);                                %update detector index
    end                                          %end loop on vector
end                                              %end check if cell

t(1) = p.tr(1)^2;                                %amplitude loss coeff.
M = M*0.5;                                       %Change mode number to M/2
for j =1:nd                                      %loop on partitions
    n1=maxm^(j-1);                               %array size below j
    n2=maxm^(nd-j);                              %array size above j
    epC(1,:) = epC(1,:) + M(j)*log(p1(j));       %m = 0 bin. coeff. product
    C = reshape(C,n1,maxm,n2);                   %reshape log prob.
    C(:,1,:) = C(:,1,:) + epC(1,:) +...
               log(hypergeom([0.5,M(j)],0.5,(1-t)^2.*(1-p1(j))));
    lp2 = log(1-p1(j));                          %get log(1-n) for j

    for m = 1:maxm*0.5                           %start loop on counts
        ehg = log(hypergeom([m + 0.5, M(j) + m],0.5,(1-t)^2.*(1-p1(j)))); 
        ohg = log(hypergeom([m + 0.5, M(j) + m],1.5,(1-t)^2.*(1-p1(j)))); 
        etm = 2*m*log(t);                        %even loss power
        otm = (2*m - 1)*log(t);                  %odd loss power

        epC(m+1,:) = epC(m,:)+log(1+(M(j)-1)/m)+lp2;%even binomial product
        if m ==1                                 %odd binomial product
            opC(m,:) = opC(m,:) + log(M(j)) + lp2;
        else 
            opC(m,:) = opC(m-1,:) + log(1+(M(j)-1)/m)+lp2;
        end 
        
        C(:,2*m,:) = C(:,2*m,:) + M(j)*log(p1(j)) +...
            log(2*m) + otm + ohg + opC(m,:) + log(1-t);%add odd count prob.
        if m < maxm*0.5
            C(:,2*m+1,:) = C(:,2*m+1,:) + etm + ehg + epC(m+1,:);%add even
        end 
    end
epC = zeros(maxm,1);
opC = zeros(maxm,1);
end                                              %start loop on counts

C = reshape(C,[sz,1]);                           %reshape log prob.
C = exp(C);                                      %probability
in = [p.xk{p.noutput}{:},{0},{0},{0},{0},{0}];
C = C(1+in{1}(1):1+in{1}(end),1+in{2}(1):1+in{2}(end),...
    1+in{3}(1):1+in{3}(end),1+in{4}(1):1+in{4}(end),...
    1+in{5}(1):1+in{5}(end),1+in{6}(1):1+in{6}(end));%treats up to 6D binning
end                                              %end nnc
